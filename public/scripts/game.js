...
function spawnEnemy() {
    ...
    if (enemyy > canvas.height - drake.height) {
        enemyy = canvas.height - drake.height;
    } else if (y <= 0) {
        enemyy = 0;
    }

    if (((x < enemyx && enemyx < x + drake.width) || (enemyx < x && x < enemyx + drake.width) ) && ( (y < enemyy && enemyy < y + drake.height) || (enemyy < y && y < enemyy + drake.height))) {
        console.log("game over!!");
    }

    if (coincount <= 5 && bossEaten == 0) {
        drake.src = "img/drakebigger.PNG"
        ctx.drawImage(drake, enemyx, enemyy);
        
    } else if (coincount > 5 && coincount <= 10) {

        drake.src = "img/taylor.PNG"
        ctx.drawImage(drake, enemyx, enemyy)
    }
}
